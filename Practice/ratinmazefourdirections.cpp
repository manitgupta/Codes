...
//Variant of Geeksforgeeks problem where mouse is allowed to move in
//4 directions

bool isSafe(int x, int y, int maze[][100], int N, int sol[][100])
{
	if(x >= 0 && x < N && y>= 0 && y < N && maze[x][y] != 0 && sol[x][y] != 1)	//last condiition: eliminates position from which we came
		return true;
	return false;
}
