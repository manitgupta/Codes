...
int Pf[N][N]; 		//Global DP Table made here for accomodating PrintDpTable() and PrintOptimalItems() functions.
					//2-D Structure to contruct the DP table
...
void PrintOptimalItems(int W, int wt[],int val[],int n)
{
	int i=n,j=W;
	while (i>0)
	{
		if (j >= wt[i-1])
		{
			if(Pf[i][j] - Pf[i-1][j-wt[i-1]] == val[i-1])
			{
				cout<<"Item "<<i<<" of Weight: "<<wt[i-1]<<" and Value: "<<val[i-1]<<" is in optimal Knapsack"<<endl;
				j = j - wt[i-1];
				i = i-1;
			}
		}	
		else
			i = i -1; //Difference was not equal to price/value of item. Hence it must be equal to preceding 
	}
}
